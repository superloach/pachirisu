...
function crew_package() {
	...
	if ! [ -e "${PREFIX}/${CHECK}" ]; then
		log "${PKG} didn't provide ${CHECK}... hmm..."
		return 1
	fi
}
...
function setup_pacman() {
	...
	meson setup \
		--prefix="${PREFIX}" \
		-D 'i18n=false' \
		-D "scriptlet-shell=${PREFIX}/bin/bash" \
		-D "ldconfig=${PREFIX}/sbin/ldconfig" \
		-D "root-dir=/" \
...
function main() {
	...
	# makepkg build deps
	crew_package 'fakeroot' 'bin/fakeroot'
	crew_package 'gnupg' 'bin/gpg'
	crew_package 'npth' 'lib64/libnpth.so'
	crew_package 'ca_certificates' 'etc/ssl/certs/ca-certificates.crt'
	crew_package 'git' 'bin/git'
	crew_package 'filecmd' 'bin/file'
	crew_package 'make' 'bin/make'
	crew_package 'libksba' 'lib64/libksba.so'
